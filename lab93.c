...
void FindMax(){
    for(int x = 0; x < 320; x++){
       // LCDLine(x,CAMHEIGHT,x,CAMHEIGHT-hist_col[x]/3,GREEN);
    ...
    for(int y = 0; y < 240; y++){
              //  LCDLine(CAMWIDTH,y,CAMWIDTH-hist_row[y]/3,GREEN);
...
int main(){
 ...
 while(button == KEY2) {
            while(button != KEY1){
                ...
                FindMax();
                IPCol2Gray(img, gray);
                IPOverlayGray(img, binary, RED, red);
                LCDImage(red);
		LCDSetPrintf(14,0,"test\n");
                button = KEYRead();
		 if(max_x < 120 && angle < 255){
                                        angle += 5;
                                        SERVOSet(1, angle);
                                }
                                else if(max_x > 200 && angle >= 5){
                                        angle -= 5;
                                        SERVOSet(1, angle);}
